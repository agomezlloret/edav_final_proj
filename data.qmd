...
Question 6: 
```{r}
marathon_data_clean_filtered <- marathon_data_clean |> 
  filter(gender != "X") 

ggplot(marathon_data_clean_filtered, aes(x = overallPlace, fill = gender)) +
  geom_density(alpha = 0.5) +
  labs(title = "Density of Finish Place by Gender",
       x = "Overall Finish Place",
       y = "Density") +
...
Question 7:
```{r}
top_countries <- marathon_data_clean %>%
  count(countryCode) %>%
  top_n(10, n)  # Get top 10 countries by count

ggplot(top_countries, aes(x = reorder(countryCode, -n), y = n)) +
  geom_bar(stat = "identity", fill = "lightblue", color = "darkblue") +
  labs(title = "Top 10 Countries Participation", 
       x = "Country", 
